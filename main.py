...
class HelpView(View):
    ...
    def main_embed(self):
        ...
        embed.add_field(name="Clans", value="Compete with groups", inline=True)
        embed.set_footer(text=f"Use {prefix}help with buttons below")
        return embed
    ...
    async def clans_btn(self, interaction: discord.Interaction, button: Button):
        prefix = get_prefix_for_guild(self.ctx.guild.id) if self.ctx.guild else GaraConfig.PREFIX
        embed = gara_embed(title="Clan Commands", color=GaraConfig.EMBED_COLOR_ACCENT, guild_id=self.ctx.guild.id if self.ctx.guild else None)
        cmds = (
